...
export class NetworkSettle {
    ...
    /** The view's verdict at exit. Pure — safe to call more than once. */
    resolve(now: number = Date.now()): LoadingTimeResult {
        ...
        if (this.settledAt === undefined) {
            // Never settled. Capped and abandoned are different populations and are read as
            // different series, so they must not be merged.
            return {
                loadingTime: null,
        ...
        const settleAt = Math.max(this.settledAt, this.runtimeReadyAt ?? 0);
    ...
    private freezeIfQuiet(now: number): void {
        if (this.settledAt !== undefined) return;
        if (this.awaitingRuntimeReady) return;   // initial_load is busy until the marker
        if (this.inFlight > 0 || this.lastEnd === undefined) return;
        if (now - this.lastEnd >= QUIET_WINDOW_MS) this.settledAt = this.lastEnd;
    }
}
